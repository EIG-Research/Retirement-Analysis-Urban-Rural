...
# Income deciles
      income_decile <- sipp_2023 %>%
        ...
        drop_na(YEAR_INC_QT) %>%
        mutate(top_income_decile = as.numeric(YEAR_INC_QT == 10) * WPFINWGT,
               bottom_income_decile = as.numeric(YEAR_INC_QT == 1) * WPFINWGT)
      ...
      income_decile_rural_urban <- income_decile %>%
        group_by(METRO_STATUS) %>%
        summarise(across(c(top_income_decile, bottom_income_decile), ~ sum(.x)/sum(WPFINWGT)))
